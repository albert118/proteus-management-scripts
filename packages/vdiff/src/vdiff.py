...
"""
A tool to help validate video stream re-encode quality without manually checking them by eye.
This check the given video stream for issues using a few strategies.
...
def analyze_video_segment(original, reencoded, ffmpeg_path, start_time, duration, worker_id, approx_frames, thread_limit):
    """
    ...
    # Clean old leftover state files if present
    ...
    # Monitor the progress file in RAM while the background process runs
        ...
        if os.path.exists(progress_file):
            try:
                with open(progress_file, "r") as f:
                    content = f.read()
                matches = frame_matcher.findall(content)
                if matches:
                    pbar.n = int(matches[-1])
                    pbar.refresh()
            except (IOError, ValueError):
                pass  # Ignore temporary read/write collisions

    pbar.close()

    # Safely drain stderr buffers all at once since processing has completed
    _, stderr_data = process.communicate()

    # Cleanup the file from RAM disk
    if os.path.exists(progress_file):
        try:
            os.remove(progress_file)
        except OSError:
...
def analyze_video_similarity_parallel(original, reencoded, ffmpeg_path, output_log, max_cores=32, num_workers=4):
    ...
    total_duration = media_utils.get_video_duration(original, ffmpeg_path)
    if not total_duration:
        print("Error: Could not determine video duration. Falling back to linear mode.")
        return None

    total_frames = media_utils.get_total_frames(original, ffmpeg_path)
    if not total_frames:
    ...
    # Map num_workers directly to the maximum concurrency limit of the ThreadPoolExecutor
    with concurrent.futures.ThreadPoolExecutor(
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                res = future.result()
                if res and "vmaf" in res:
                    segment_results.append(res)
            except Exception as e:
                print(f"[!] Error processing worker task: {e}")
    ...
    if not segment_results:
        print("[!] Error: No valid metrics captured from workers.")
        return {}

    # Calculate final averages across all chunks
    final_results = {
        "ssim_global": sum(c["ssim_global"] for c in segment_results)
        / len(segment_results),
        "ssim_y": sum(c["ssim_y"] for c in segment_results)
        / len(segment_results),
        "ssim_cb": sum(c["ssim_cb"] for c in segment_results)
        / len(segment_results),
        "ssim_cr": sum(c["ssim_cr"] for c in segment_results)
        / len(segment_results),
        "vmaf": sum(c["vmaf"] for c in segment_results) / len(segment_results),
        "psnr_y": sum(c["psnr_y"] for c in segment_results)
        / len(segment_results),
        "psnr_average": sum(c["psnr_average"] for c in segment_results)
        / len(segment_results),
    }
